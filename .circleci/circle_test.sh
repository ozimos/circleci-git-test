...
LATEST_PACKAGE_COMMIT_SINCE_DEVELOP=$(git rev-list -1 $DEVELOP_SHA1..$CIRCLE_SHA1 -- ${PACKAGE_PATH#./})
LATEST_COMMIT_SINCE_DEVELOP=$(git rev-list -1 $DEVELOP_SHA1..$CIRCLE_SHA1)
COMMITS_SINCE_DEVELOP=$(git rev-list $DEVELOP_SHA1..$CIRCLE_SHA1)
COMMITS_COUNT_SINCE_DEVELOP=$(git rev-list --count $DEVELOP_SHA1..$CIRCLE_SHA1)
...
echo -e "\e[36m  sha1 at develop HEAD $DEVELOP_SHA1 \e[0m"
